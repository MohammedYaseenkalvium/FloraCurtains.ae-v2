    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={label} className="bg-flora-surface rounded-xl p-4 border border-flora-border">
            <div className="text-[10px] uppercase tracking-widest text-flora-muted mb-1">{label}</div>
      ...
      <div className="border border-flora-border rounded-xl overflow-hidden mb-4">
        <table className="w-full text-sm">
          ...
          <tbody>
            {payments.length === 0 ? (
              <tr><td colSpan={6} className="p-4 text-center text-flora-muted text-sm">No payments recorded yet.</td></tr>
            ) : payments.map(p => (
              <tr key={p.id} className="border-t border-flora-border/60">
                ...
                <td className="p-3 font-semibold text-flora-primary">AED {p.amount.toLocaleString()}</td>
                <td className="p-3 text-flora-muted">{p.reference ?? "—"}</td>
                <td className="p-3 text-flora-muted">{p.notes ?? "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {/* Add Payment */}
      {!open ? (
        <button onClick={() => setOpen(true)} className="flex items-center gap-2 text-flora-primary text-sm hover:underline">
          <PlusCircle size={15} /> Record Payment
        </button>
      ) : (
        <div className="border border-flora-border rounded-xl p-4 space-y-3">
          <h4 className="font-semibold text-sm text-flora-primary">Record Payment</h4>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-[10px] uppercase tracking-widest text-flora-muted block mb-1">Amount (AED) *</label>
              <input type="number" className={field} value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} />
            </div>
            <div>
              <label className="text-[10px] uppercase tracking-widest text-flora-muted block mb-1">Type</label>
            ...
            <div>
              <label className="text-[10px] uppercase tracking-widest text-flora-muted block mb-1">Method</label>
            ...
            <div>
              <label className="text-[10px] uppercase tracking-widest text-flora-muted block mb-1">Reference / Cheque No.</label>
              <input className={field} value={form.reference} onChange={e => setForm(f => ({ ...f, reference: e.target.value }))} />
            </div>
            <div className="col-span-2">
              <label className="text-[10px] uppercase tracking-widest text-flora-muted block mb-1">Notes</label>
